...
def text_to_speech():
    try:
        ...
        response = send_file(
            audio_buffer,
            mimetype='audio/mpeg',
            as_attachment=False, # Quan trọng để trình duyệt/client có thể phát trực tiếp
            download_name='speech.mp3'
        )

        # Thêm headers cho CORS nếu cần (đã có)
...
if __name__ == '__main__':
    # Khi chạy cục bộ, Vercel CLI hoặc lệnh `python app.py` sẽ khởi động server.
    # Port 5000 là ví dụ, Vercel sẽ quản lý port khi triển khai.
